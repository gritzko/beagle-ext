...
function decide(be, wtlogReader, storeReader, narrow) {
  ...
  for (const path of paths) {
    ...
    //  Gitlink with no bump.  An explicit `be put` that now finds a REGULAR
    //  file / symlink / exec on disk REPLACES the submodule with that blob (a
    //  gitlink → file/symlink conversion, e.g. a `be -> .` self-mount link);
    //  the sub vanishes, so DON'T record its prefix.  With no such put+blob,
    //  carry the gitlink through verbatim (no on-disk file to compare).
    if (b && b.mode === MODE.s) {
      if (p && w) {
        const mode = MODE[w.kind] || MODE.f;
        const sha = hashWtPath(wtRoot, path, w.kind);
        if (sha) { add(path, mode, sha, b.sha); continue; }
      }
